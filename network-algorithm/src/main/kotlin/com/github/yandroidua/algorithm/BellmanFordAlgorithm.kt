...
class BellmanFordAlgorithm(
   ...
   private fun calculatePaths(
      ...
      repeat(workstations.size - 1) {
         repeat(lines.size) { lineIndex ->
            ...
            if (firstWorkStation?.weight != or && (secondWorkStation?.weight ?: -1) > (firstWorkStation?.weight?.plus(
                  weight
               ) ?: -1)
            ) {
               secondWorkStation?.weight = firstWorkStation?.weight?.plus(weight) ?: -1
            } else if (secondWorkStation?.weight != or && (firstWorkStation?.weight
                  ?: -1) > (secondWorkStation?.weight?.plus(weight) ?: -1)
            ) {
   ...
   private fun calculateMinPath(
      to: PathHolder,
      from: Workstation,
      or: Int,
      distances: Array<PathHolder>,
      byLength: Boolean = true
   ) {
