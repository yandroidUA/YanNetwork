...
class RoutingTable(
   ...
   fun routingTable(pathRequester: (from: WorkstationId, to: WorkstationId) -> SimulationPath?): List<SimulationRoutingTableEntry> {
      ...
      // if workstation not in network it should know at least about self and neighbours
...
//         val neighbour = allElements.find { it.id == neighbourId } as? SimulationWorkstation ?: continue
...
//      if (workstation.network.isNullOrBlank()) return result
      ...
      val networkElements = allElements.asSequence()
         ...
         .filter { w -> w.isWorkstation && !result.any { it.toId == w.id } }
      ...
      for (networkElement in networkElements) {
         ...
         result.add(
            SimulationRoutingTableEntry(
               toId = networkElement.id,
               fromId = nextElement.second
            )
         )
      }
      return result
